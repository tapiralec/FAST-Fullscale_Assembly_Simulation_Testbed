...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public bool useHoverFancy = true;
        public float  capsuleRadius = .053f;
        public float  capsuleHeight = .013f;
        public Vector3 capsuleCenter = new Vector3(0f, -.044f, 0f);
        public int capsuleDirection = 0;
        public Vector3 boxCenter = new Vector3(0f, -.01613167f, -.08370697f);
        public Vector3 boxScale = new Vector3(.05845841f, .04772133f, .179614f);
        ...
        public InteractableVOTE interactableVote;
        ...
        private Interactable _votedInteractable;
        ...
        public Interactable hoveringInteractable
        {
            ...
            //get { return interactableVote.EvaluateVote(); }
            set
            {
        ...
        {
            // nasty way to lock something to the hand - Imperative to reset this to false later or it
            // will completely break all subsequent interactions.
            if (isLocked) return;
            ...
            if (index != -1)
            {
                ...
                Debug.Log($"[Detaching from Hand] index={index}, attachedObjects.Count={attachedObjects.Count}");
        ...
        protected virtual void UpdateHovering()
        {
            ...
            if (useHoverFancy)
            {
                Vector3 capP0 = transform.position + transform.rotation *(
                    capsuleCenter +
                    (capsuleDirection==0 ? Vector3.right : capsuleDirection==1 ? Vector3.up : Vector3.forward)
                    * 0.5f * capsuleHeight);
                Vector3 capP1 = transform.position + transform.rotation *(
                    capsuleCenter +
                    (capsuleDirection==0 ? Vector3.right : capsuleDirection==1 ? Vector3.up : Vector3.forward)
                    * -.5f * capsuleHeight);
                Vector3 capMid = transform.position + transform.rotation * capsuleCenter;
                Vector3 currentBoxCenter = transform.position + transform.rotation * boxCenter;
                // Check both the capsule and the box - note that we're passing in the center of the capsule transform as
                // the "closestPosition" argument for the Box, this allows us to check the distance to the center of the
                // capsule for finding closest, instead of using the middle of the box which would yield ambiguity.
                CheckHoveringForTransform_Capsule(capMid, capP0, capP1, ref closestDistance, ref closestInteractable, Color.black);
                CheckHoveringForTransform_Box(currentBoxCenter, transform.rotation, capMid, ref closestDistance, ref closestInteractable, Color.black);
            ...
            if (interactableVote != null)
            {
                interactableVote.VoteForInteractable(closestInteractable);
                hoveringInteractable = interactableVote.EvaluateVote();
            }
            else
            {
                // Hover on this one
        ...
        protected virtual bool CheckHoveringForTransform_Capsule(Vector3 capsuleCenter, Vector3 p0, Vector3 p1,
            ref float closestDistance, ref Interactable closestInteractable, Color debugColor)
        {
            ...
            int numColliding = Physics.OverlapCapsuleNonAlloc(p0,p1, capsuleRadius,overlappingColliders, hoverLayerMask.value);
            ...
            for (int colliderIndex = 0; colliderIndex < overlappingColliders.Length; colliderIndex++)
            {
                ...
                float distance = Vector3.Distance(contacting.transform.position, capsuleCenter);
            ...
            {
                Debug.DrawLine(capsuleCenter, closestInteractable.transform.position, debugColor, .05f, false);
            }
        ...
        protected virtual bool CheckHoveringForTransform_Box(Vector3 currentBoxCenter, Quaternion orientation, Vector3 closestPosition,
            ref float closestDistance, ref Interactable closestInteractable, Color debugColor)
        {
            bool foundCloser = false;
            // null out old vals
            ...
            int numColliding = Physics.OverlapBoxNonAlloc(currentBoxCenter, 0.5f*boxScale, overlappingColliders, orientation, hoverLayerMask.value);
            //int numColliding = Physics.OverlapSphereNonAlloc(hoverPosition, hoverRadius, overlappingColliders, hoverLayerMask.value);
            ...
            for (int colliderIndex = 0; colliderIndex < overlappingColliders.Length; colliderIndex++)
            {
                ...
                // Best candidate so far...
                float distance = Vector3.Distance(contacting.transform.position, closestPosition);
                //float distance = Vector3.Distance(collider.bounds.center, hoverPosition);
                bool lowerPriority = false;
                if (closestInteractable != null)
                { // compare to closest interactable to check priority
            ...
            if (showDebugInteractables && foundCloser)
            {
                Debug.DrawLine(closestPosition, closestInteractable.transform.position, debugColor, .05f, false);
            }
        ...
        protected virtual void Update()
        {
            ...
            //ALEC: Yes, this is gross.
            //interactableVote.VoteForInteractable(_hoveringInteractable);
        ...
        public bool IsStillHovering(Interactable interactable)
        {
            if (interactableVote != null)
            {
                return interactableVote.EvaluateVote() == interactable;
            }
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            if (useHoverFancy && mainRenderModel != null && mainRenderModel.IsControllerVisibile())
            {
                Gizmos.matrix = Matrix4x4.TRS(transform.position,transform.rotation,Vector3.one);
                Gizmos.color = Color.black;
                Gizmos.DrawWireCube(boxCenter, boxScale);
                Gizmos.DrawWireSphere(capsuleCenter +
                    (capsuleDirection==0 ? Vector3.right : capsuleDirection==1 ? Vector3.up : Vector3.forward)
                    * -.5f * capsuleHeight,
                    capsuleRadius);
                Gizmos.DrawWireSphere(capsuleCenter +
                ...
                Gizmos.color = Color.black;
                // Could set up fancy debug lines here...
                //Gizmos.DrawLine(
                    //boxCenter - Vector3.Scale(Vector3.left,boxScale) - Vector3.Scale(Vector3.up,boxScale) - Vector3.Scale(Vector3.forward,boxScale),
                    //boxCenter - Vector3.Scale(Vector3.left,boxScale) - Vector3.Scale(Vector3.up,boxScale) - Vector3.Scale(Vector3.forward,boxScale)
                    //);
                Gizmos.matrix = Matrix4x4.identity;
            }
